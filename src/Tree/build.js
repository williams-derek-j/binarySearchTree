...
export default function build(array, parent) {
    ...
    if (array.length === 1) {
        ...
        if (array[0] < parent.value && array[1] > parent.value) { // sorted already
            ...
            if (array[0] < parent.value) { //
                ...
                if (array[1] !== parent.value) { // check for duplicate as fail condition
                    this.build([array[1]], parent.left) // both less than parent; parent.left.left
                } else {
                ...
                this.build([array[1]], parent.right) // parent.right.right
            } else { // element === parent, skip
                if (array[0] !== array[1]) {  // both equal parent?
                    this.build([array[1]], parent) // skip duplicate element, append next
                }
            }
            ...
            if (array[1] < parent.value) {
                ...
                if (array[0] !== parent.value) { // check for duplicate as fail condition
                    this.build([array[0]], parent.left) // both less than parent; parent.left.left
                } else {
                    // could return a leaf node up the recursion chain here
                }
                ...
                this.build([array[0]], parent.right) // parent.right.right
            } else { // element === parent, skip
                if (array[1] !== array[0]) { // both equal parent?
                    this.build([array[0]], parent) // skip duplicate element, append next
                }
            }
        } else if (array[0] !== array[1]) { // check if both values same as fail condition
            this.build([array[0]], parent) // remove duplicate and try again
        }
        ...
        if (array[midpoint] < parent.value) {
            ...
            this.build(array.slice(0, midpoint), parent.left)
            this.build(array.slice(midpoint + 1), parent.left)
        } else if (array[midpoint] > parent.value) {
            parent.right = new Node(array[midpoint])

            this.build(array.slice(0, midpoint), parent.right)
            this.build(array.slice(midpoint + 1), parent.right)
        } else if (array[midpoint] === parent.value) {
